...
Steps are fenced code blocks with `bash`, `sh`, or `powershell` language tag:
...
State saved to `.runbook/state/<runbook-name>.json`:
```json
...
- **State:** `.runbook/state/` JSON files
...
- Non-bash script steps (PowerShell only if shell is bash-compatible)
